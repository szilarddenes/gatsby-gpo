...
app.post('/apiMail', (req, res) => {
    ...
    function pushMail() {
        ...
        function saveToDb() {
            ...
            fs.readFile(db, 'utf8', function readFileCallback(err, resObj) {
                if (err) {
                    console.log(err);
                } else {
                    obj = JSON.parse(resObj); //now it an object
                    obj.table.push(sendData); //add some data
                    json = JSON.stringify(obj); //convert it back to json
                    fs.writeFile(db, json, 'utf8', callback); // write it back 
                }
            });
